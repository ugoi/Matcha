  ...
  -- Users Table: Handles user registration and login (private data)
CREATE TABLE users (
    user_id UUID PRIMARY KEY DEFAULT gen_random_uuid(), -- UUID as primary key using gen_random_uuid()
    username TEXT UNIQUE NOT NULL, -- No length restriction, TEXT is equivalent to VARCHAR without limit
    email TEXT UNIQUE NOT NULL, -- No length restriction
    password_hash TEXT NOT NULL, -- Store the hashed password
    first_name TEXT,
    last_name TEXT,
    is_active BOOLEAN DEFAULT TRUE, -- To mark if the user is active
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
