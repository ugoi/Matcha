...
export default function Chat() {
  ...
  const [expandedProfile, setExpandedProfile] = useState<FullProfile | null>(
    null
  );
    ...
    const markAllAsRead = async () => {
      try {
        ...
        if (
          ...
          const unreadIds = result.data.notifications
            .filter((n: any) => n.status !== "2")
            .map((n: any) => n.id);
  ...
  useEffect(() => {
    if (!userId) return;
    socketRef.current = io(`${window.location.origin}/api/chat`, {
      withCredentials: true,
    });
    socketRef.current.on(
      "chat message",
      (data: ChatMessageData, ack: (response: string) => void) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      const [matchedRes, likesRes] = await Promise.all([
        fetch(`${window.location.origin}/api/profiles/matched`, {
          credentials: "include",
        }),
        fetch(`${window.location.origin}/api/profiles/likes`, {
          credentials: "include",
        }),
      ]);
      ...
      if (
        matchedData.status === "success" &&
        Array.isArray(matchedData.data?.matches)
      ) {
      ...
      if (
        ...
        Array.isArray(likesData.data?.likes)
      ) {
        const arr = likesData.data.likes
          .map((like: any) => {
            if (
              userId === like.likee_user_id &&
              like.matcher &&
              like.matcher.length > 0
            ) {
              ...
              return {
                ...
                profile_picture:
                  u.profile_picture || "https://via.placeholder.com/40",
              };
            }
            return null;
          })
  ...
  useEffect(() => {
    if (!userId) return;
    (async () => {
      const res = await fetch(`${window.location.origin}/api/profiles/visits`, {
        credentials: "include",
      });
      const data = await res.json();
  ...
  const openChat = async (uid: string) => {
    ...
    try {
      const res = await fetch(
        `${window.location.origin}/api/chats/${uid}?limit=20`,
        { credentials: "include" }
      );
  ...
  const fetchFullProfile = async (userId: string) => {
    try {
      const res = await fetch(
        `${window.location.origin}/api/profiles/${userId}`,
        { credentials: "include" }
      );
      const data = await res.json();
      return data.status === "success" && data.data
        ? (data.data as FullProfile)
        : null;
    } catch (error) {
      return null;
    }
  ...
  const handleExpandView = async (viewItem: any) => {
    ...
    if (viewItem.visitor_profile && viewItem.visitor_profile.length > 0) {
      ...
      viewItem.visited_profile &&
      viewItem.visited_profile.length > 0
    ) {
  ...
  const handleReportFake = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${window.location.origin}/api/profiles/${expandedProfile.profile_id}/report`,
        {
          method: "POST",
  ...
  const handleNextExpandedPhoto = () => {
    ...
    const orderedPictures = expandedProfile.profile_picture
      ? [
          { picture_url: expandedProfile.profile_picture },
          ...(expandedProfile.pictures
            ? expandedProfile.pictures.filter(
                (p) => p.picture_url !== expandedProfile.profile_picture
              )
            : []),
        ]
      : expandedProfile.pictures || [];
  ...
  const handlePreviousExpandedPhoto = () => {
    ...
    const orderedPictures = expandedProfile.profile_picture
      ...
      : expandedProfile.pictures || [];
    if (!orderedPictures.length) return;
    setExpandedPhotoIndex(
      (prev) => (prev - 1 + orderedPictures.length) % orderedPictures.length
    );
  };
  ...
  const renderExpandedProfile = () => {
    ...
    const orderedPictures = expandedProfile.profile_picture
      ...
      : expandedProfile.pictures || [];
    const currentPhoto = orderedPictures.length
      ? orderedPictures[expandedPhotoIndex]?.picture_url
        ...
        >
          ...
          <div className="card-body">
            <h4 className="card-title mb-2">
              {expandedProfile.first_name} {expandedProfile.last_name},{" "}
              {expandedProfile.age}
            </h4>
            <p className="card-text text-muted mb-3">
              {expandedProfile.biography}
            </p>
            {expandedProfile.interests &&
            ...
            <p className="card-text text-muted mb-1">
              Fame Rating: {expandedProfile.fame_rating}
            </p>
  ...
  const handleUnmatch = async () => {
    ...
    try {
      await fetch(
        `${window.location.origin}/api/profiles/${selectedUserId}/like`,
        {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
  ...
  const handleBlock = async () => {
    ...
    try {
      await fetch(
        `${window.location.origin}/api/profiles/${selectedUserId}/block`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
            ...
            likes.map((like) => (
              ...
              >
                <img
                  ...
                  alt={`${
                    (like.first_name + " " + (like.last_name || "")).trim() ||
                    "Unknown"
                  }`}
                ...
                />
                <span>
                  {`${like.first_name} ${like.last_name}`.trim() || "Unknown"}
                </span>
              </li>
            ))
          ) : (
